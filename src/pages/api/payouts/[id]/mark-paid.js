...
import { markPayoutPaid } from '@/prisma/services/payout';
...
const handler = async (req, res) => {
  ...
  if (method === 'POST') {
    try {
      ...
      const { paidAt, reference } = req.body;

      const statement = await markPayoutPaid(id, paidAt || new Date(), reference);
      ...
      console.error('[api/payouts/[id]/mark-paid] Error:', error);
      if (!res.headersSent) {
        const msg = String(error?.message || 'Failed to mark payout as paid');
