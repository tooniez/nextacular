...
import { issuePayoutStatement } from '@/prisma/services/payout';
...
const handler = async (req, res) => {
  ...
  if (method === 'POST') {
    try {
      ...
      const statement = await issuePayoutStatement(id);
      ...
      console.error('[api/payouts/[id]/issue] Error:', error);
      if (!res.headersSent) {
        const msg = String(error?.message || 'Failed to issue payout statement');
