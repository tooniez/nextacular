...
export default async function handler(req, res) {
  ...
  try {
    fetch('http://localhost:7242/ingest/63d3e4d3-5a4a-4343-8839-58d002db9a84', {
      ...
      body: JSON.stringify({
        ...
        location: 'src/pages/api/_debug/subcpo-permissions-audit.js',
  ...
  try {
    const path = '/opt/nextacular/SUBCPO_PERMISSIONS_AUDIT.md';
    const content = await fs.readFile(path, 'utf8');

    res.setHeader('Content-Type', 'text/markdown; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="SUBCPO_PERMISSIONS_AUDIT.md"');
    ...
    // #endregion
    return res.status(500).json({ errors: { error: { msg: 'Failed to read SUBCPO_PERMISSIONS_AUDIT.md' } } });
  }
}
